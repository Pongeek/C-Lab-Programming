...
/* tracks the current state of the program as it reads and processes user input.*/
...
/* array of all 7 valid command names */
static char* commands[] = { "read_set","print_set", "union_set",
						"intersect_set", "sub_set","symdiff_set", "stop" };
...
int main(void)
{
	...
	printf("hello ladies and gentlemen!, and welcome to SET OPERATIONS.\n\n");
	printf("Let's play around with sets by using few operations on them.\n\n");
	printf("1. Valid set names are only: SETA, SETB, SETC, SETD, SETE, SETF.\n\n");
	printf("2. Valid opertaions only: 'read_set', 'print_set', 'union_set', ");
	printf("'intersect_set', 'sub_set' and 'symdiff_set'\n\n");
	printf("3. Valid range of numbers: 0-127 inc.\n\n");
	printf("4. To use function 'read_set', input as follows : \n\n");
	printf("'read_set <setname>,<number>,...,<number>,-1' then press <ENTER>\n\n");
	printf("for example: read_set SETA, 1, 2 ,3 ,-1\n\n");
	printf("5. To perform 'print_set', input as follows:\n\n");
	printf("'print_set <setname>' then press <ENTER>\n\n");
	printf("for example: 'print_set SETA'\n\n");
	printf("6. To use fucntion union/intersect/sub/symdiff _set, input as follows:\n\n");
	printf("'<operation> <setname1>,<setname2>,<setname3>' then press <ENTER>\n\n");
	printf("for example: 'intersect_set SETB, SETF, SETD'\n\n");
	printf("NOTE: to print the solution of the operation from example above, write 'print_set SETC'");
	printf("NOTE: the same set can be used twice or 3 times ");
	printf("in uni/inter/sub/symdiff operations.\n\n");
	printf("7. to quit: 'stop'and then press <ENTER>\n\n");
	...
	while (fgets(command_line, MAX_LEN, stdin) != NULL)
	{
		...
		if (vldcmdln(cmdln))
		{
			...
			case 1:
				...
				/* union_set, interset_set, sub_set, symdiff_set */
	...
	switch (state)
	{
		...
		case COMMA_AFTER_SET1:
			...
			else
			{
				...
				/* otherwise inter/uni/sub/symdiff cmd */
				else
				{
					state = SET2;
				}
